...
function installShim() {
  ...
  // Mirror prefers-color-scheme → [theme='dark'] on :root so that
  // :root[theme='dark'] CSS selectors in app.less work in local web preview
  // the same way they do when the Tuya App sets the attribute on the phone.
  function syncThemeAttr(dark: boolean) {
    if (dark) {
  ...
  if (colorSchemeMq) {
    syncThemeAttr(colorSchemeMq.matches);
    colorSchemeMq.addEventListener('change', (e: MediaQueryListEvent) => syncThemeAttr(e.matches));
  }
